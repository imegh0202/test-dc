...
In Discord, open the target channel settings, then create a webhook under `Integrations`. Discord's official webhook docs are here:
https://docs.discord.com/developers/platform/webhooks

For this project:

1. Create a webhook in the target channel
2. Copy the full webhook URL
3. Keep the full URL private
4. Put it in `.env` or GitHub Secret `DISCORD_WEBHOOK_URL`
...
- If Discord returns `401 Unauthorized`, the webhook is not authorized and should be regenerated.
- If Discord returns `403 Forbidden`, the webhook is usually invalid, disabled, or leaked.
- If Discord returns `404 Not Found`, the webhook URL is wrong or the webhook was deleted.
